...
int stm;
 ...
 Soundpool pownsounds = Soundpool(streamType: StreamType.notification);

sound({String audio})async{
...
sounds({String audio})async{
  int soundId = await rootBundle.load(audio).then((ByteData soundData) {
    return pownsounds.load(soundData,);
  });
  stm = await pownsounds.play(soundId,);
}

sound3({String audio})async{
  int soundId = await rootBundle.load(audio).then((ByteData soundData) {
    return pownsound.load(soundData);
  });
